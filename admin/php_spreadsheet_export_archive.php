...
$sql = "SELECT *, employees.id AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id WHERE reason IS NOT NULL ORDER BY lastname DESC";
...
if(isset($_POST["export"]))
{
  ...
  $active_sheet->setCellValue('H1', 'AGE');
  ...
  $active_sheet->setCellValue('R1', 'DATE DELETED');
  $active_sheet->setCellValue('S1', 'CAUSE OF DELETION');
  ...
  foreach($result as $row)
  {
    ...
    $active_sheet->setCellValue('J' . $count, $row["gsis"]);
    ...
    $active_sheet->setCellValue('R' . $count, date('M d, Y', strtotime($row['deleted_on'])));
    $active_sheet->setCellValue('S' . $count, $row["reason"]);
...
<html>
   <head>
     <title>Archive - Export to Excel File</title>
   ...
   <body>
     <div class="container">
      <br />
      <h3 align="center"><b>EXPORT EMPLOYEE ARCHIVE FROM WEBPAGE TO EXCEL FILE</b></h3>
      <br />
           ...
           <table class="table table-striped table-bordered">
                ...
                foreach($result as $row)
                {
                  ...
                  <tr>
                    ...
                    <td>'.date('M d, Y', strtotime($row['deleted_on'])).'</td>
                    <td>'.$row["reason"].'</td>
                  </tr>
                  ';
                }
